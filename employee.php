...
$tsql = "SELECT SpecialOrders.[DateTime]
  ...
  WHERE SpecialOrders.[DateTime] < Convert(datetime, '".date('Y-m-d', $newdate)."') AND SpecialOrders.[Preorder] = '0' AND SpecialOrders.[Status] = 'O' and Customer.Fax LIKE '.'";
...
echo "<br>&nbsp;&nbsp;&nbsp;<b><u>List of Employee Special Orders to be bumped to the bottom of the list.</b></u><br>";
...
$tsql1 = "SELECT [First_Name]
      ,[Last_Name]
   FROM [esc_v20].[dbo].[Customer]
  where [CustNum]='".$row[1]."'";

$cust = sqlsrv_query( $conn, $tsql1);
$custa = sqlsrv_fetch_array($cust);
...
echo "<a href=\"#\" onClick=\"SPOCleanup(2);\">Continue?</a>";
